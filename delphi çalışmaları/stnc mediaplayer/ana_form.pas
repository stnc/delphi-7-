...
type
  TForm1 = class(TForm)
    ...
    procedure CExSliderCtrl1PositionChanged(ASender: TObject;
      nPosition: Integer);
// windows un warsayýlan alanýna kendi sistem menunuzü ekler
  private
...
var
 ...
 for say:=0 to ListBox1.Items.Count do  //listboxdaki elemna sayýsýný bul
  //o dan baþla ve bunu listbox a ekelemk için gerekli for dongusu
...
//listbox 1 ve 2 ye ayný elemenlarý ekle saçilen leri


listbox1.Items.Add( (IntToStr(say)+'--'+ExtractFileName(StartDir + SR.Name)));// ExtractFileName buradan exaxt ý siliniz
listbox2.Items.Add(StartDir + SR.Name);// ExtractFileName buradan exaxt ý silini
...
procedure TForm1.Yukle_tusClick(Sender: TObject);
  ...
  if (OpenDialog1.Execute) then begin
    ...
    MediaPlayer1.Open;
   label1.Caption := 'Çalan Dosya Adý     '+ ExtractFileName(openDialog1.FileName)
   +#13+ 'Dosyanýn Yolu      '+openDialog1.FileName+#13+'Bulundugu Klasor     '+ExtractFilePath(openDialog1.FileName);
      Panel1.Caption :='';
      //not  ExtractFileExt() uzuntý kontrol eder
         Timer1.Enabled := true;
...
MediaPlayer1.Left:=(panel1.ClientWidth-MediaPlayer1.Width);   //tam ekran kodu olmasa da olur
 MediaPlayer1.DisplayRect := Rect(0 , 0, panel1.Clientwidth, panel1.ClientHeight);//tam ekran  þ
...
MediaPlayer1.Display :=panel1 ;
  ...
  sayi:=ListBox1.Items.IndexOf(listbox1.items.strings[listbox1.itemindex]);  // listbocda 1 de seçili olanýn numarasýnýu bul
...
//resim_yeri.Caption :=olay;
        ...
        //çok onemli bunlarýn açýlmasý form3 deki scrollbar da hata yapýyor sekme olmuyor
...
// Form2.WindowState :=wsMaximized;//tam ekran için once form buyutulur
 Show;
MediaPlayer1.Left:=(panel1.ClientWidth-MediaPlayer1.Width);   //tam ekran kodu olmasa da olur
 MediaPlayer1.DisplayRect := Rect(0 , 0, panel1.Clientwidth, panel1.ClientHeight);//tam ekran  þart kod
...
//form3.Timer1.Enabled := true;
 ...
 //ShowMessage('Number of frames = ' +
    //Edit1.Text:=('KAre sayýsý = ' + IntToStr(MediaPlayer1.Length));

  MediaPlayer1.TimeFormat :=tfMilliseconds;
   // MediaPlayer1.TimeFormat := tfMilliseconds;

 //Edit2.Text:=('süre = ' +  IntToStr(MediaPlayer1.Length));
 begin
...
begin {timer ýn interval deðeri 1  olsun bu pc'yi biraz zorlar ama}
 ScrollBar1.OnChange := nil;
  ScrollBar1.Position := MediaPlayer1.Position;
 // ProgressBar1.Position:=MediaPlayer1.Position;
  Gauge1.Progress  :=MediaPlayer1.Position;
         ProgressBar1.Position  :=MediaPlayer1.Position;
  //Label3.Caption:='% '+FloatToStr(ProgressBar1.Position*100/ProgressBar1.Max) + 'sý Tamamlandý';
...
Form1.WindowState :=wsMaximized;//tam ekran için once form buyutulur
Show;
MediaPlayer1.Left:=(panel1.ClientWidth-MediaPlayer1.Width);   //tam ekran kodu olmasa da olur
MediaPlayer1.DisplayRect := Rect(0 , 0, form1.Clientwidth,form1.ClientHeight);//tam ekran  þ
...
hwin2:thandle;
begin
asd:='Program Hakkýnda';
hwin:=form1.handle; //forma handle al
hwin2:=GetSystemMenu(hwin,false); // system menüye handle al
DeleteMenu(hwin2, sC_RESTORE, MF_BYCOMMAND );//önceki boyut elemanýnýsil
DeleteMenu(hwin2, SC_MAXIMIZE, MF_BYCOMMAND);//ekraný kapla elemanýnýsil
DeleteMenu(hwin2,SC_SIZE, MF_BYCOMMAND);    //boyut elemanýný sil
AppendMenuA(hwin2,MF_STRING,1009,asd);//program hakkýnda isminde eleman ekle
...
begin
 if Msg.CmdType =1009 then //program hakkýndaya týklandý ise
 begin
      application.Title:='Hakkinda';
      showmessage('       SYSTEMMENÜ PROGRAMÝ     '+#13+#10+#13+
     'Yazan:'+#9+ 'Selman Tunç'#13+#10+
     'Tarih:'+#9+ '17 kasým 2006'+#13+#10+
...
//olay:=(listbox1.items.strings[listbox1.itemindex]);  // list box2deki daki secimin ne oldugu nu bulur
        Font.Color  := ClBlack; //1 Numaralý Satýrýn Yazý Rengi
        Brush.Color := ClWhite; //1 Numaralý Satýrýn Zemin Rengi
        Font.Color  := ClBlack;  //0 Numaralý Satýrýn Yazý Rengi
        Brush.Color := $00D5FBFD;//0 Numaralý Satýrýn Zemin Rengi
