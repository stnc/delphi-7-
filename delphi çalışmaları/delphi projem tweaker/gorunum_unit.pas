...
Reg:=TRegistry.Create;
  ...
  reg.OpenKey ('Software\Microsoft\Windows\CurrentVersion\Policies\System',true);//kaydý acar
                 if Reg.ValueExists('NoDispSettingsPage') then   //kaydýn olup olmadýgýný kontrol eder
                 alr:=reg.ReadInteger ('NoDispSettingsPage'); //verdigimiz deðiskene gore kaydý okur
                  //varsa kaydý alr ye atar

   if  alr=1 then //eger kayýt 1 ise
   CheckBox1.Checked:=true //check box ý iþaretler

     else   //deðilse þart uymuyorsa
     CheckBox1.Checked:=false;   //check boz iþaretini kaldýrýr
     reg.CloseKey;
     reg.Free;   //bellekten kaldýrýr
 begin

Reg:=TRegistry.Create;
  reg.RootKey:=HKEY_CURRENT_USER ;
  reg.OpenKey('Software\Microsoft\Windows\CurrentVersion\Policies\Explorer',true);
         if Reg.ValueExists('NoThemesTab') then   //kaydýn olup olmadýgýný kontrol eder
                 alr2:=reg.ReadInteger ('NoThemesTab'); //verdigimiz deðiskene gore kaydý okur
   if  alr2=1 then
